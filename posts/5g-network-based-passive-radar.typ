...
#let info = (title: "5G Network Based Passive Radar", date: "2023-07-11", tags: ("radar", "startup"))
#metadata(info) <frontmatter>
#show: body => post(info: info, body)

Piotr Samczyński, Karol Abratkiewicz, Marek Płotka, Tomasz P. Zieliński, Jacek Wszołek, Sławomir Hausman, Piotr Korbel, Adam Księżyk
...
These are being shut off, and 5G networks are being spun up. Different nations use different-width 5G base stations. Poland covers up to 80 MHz, so 3.75 meters bistatic range resolution. 5 unnamed providers are up to 400 MHz, so 0.75 meters bistatic range resolution. So 1–2 orders of magnitude better than DVB-T.
...
Previous papers used software-defined radio or known ahead-of-time replays type nonsense, this paper uses the real (cooperative) McCoy. @Samczynski2022
...
3.44 GHz carrier. 30 kHz subcarrier spacing. Time Division Duplex. Lots of nuts and bolts about how long each symbol is and how they're packed.
...
5G testbed network managed by the University. 700 MHz anchor, with 5 MHz downlink and uplink. NR transceivers at 3.5 GHz, TDD 74% downlink, 20% uplink, 50 W nominal power, 40 MHz bandwidth from 3420–3460 MHz.
...
27 dB difference in noise levels depending on data content in the signal.
...
#bibliography("5g-network-based-passive-radar.bib", style: "ieee")
