...
#let info = (title: "Dual Side Scheduling for Radar Resource Management", date: "2023-01-28", tags: ("radar",))
...
- ED: earliest-deadline (scheduling method)
...
_Thought:_ A diagram is given where there is no overlap in tasks, and when moving both toward $s$ they are closer than when both moved to $0$. Not obvious, to me, why we would shift at all in the example given in Fig. 2.
...
_Thought:_ Examples shown look like just introducing a pause to start. They evaluate every candidate, I wonder if a binary search on start delay could yield similar performance in log(candidates) time would perform as well.
...
#bibliography("dual-side-scheduling-for-radar-resource-management.bib", style: "ieee")
