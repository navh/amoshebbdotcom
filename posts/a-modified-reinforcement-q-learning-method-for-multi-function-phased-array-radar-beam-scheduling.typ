...
#let info = (title: "A Modified Reinforcement Q Learning Method for Multi Function Phased Array Radar Beam Scheduling", date: "2023-01-29", tags: ("radar",))
#metadata(info) <frontmatter>
#show: body => post(info: info, body)

Kosuru, Qu, Ding, Moo @kosuru2022modified.
...
We consider $N = 8$ tasks in window $L = 1$. Cost increases as more tasks are moved. Taken from Qu, Ding, Moo "A machine learning radar scheduling method based on the EST algorithm" @qu2019machine.

==== Scheduling Algos

+ earliest start time (est)
+ priority scheduling
+ dwell time priority scheduling
+ random shifted start time (rsst)
...
#bibliography("a-modified-reinforcement-q-learning-method-for-multi-function-phased-array-radar-beam-scheduling.bib", style: "ieee")
