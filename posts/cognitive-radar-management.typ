...
#let info = (title: "Cognitive Radar Management", date: "2023-07-22", tags: ("radar",))
...
// [CHECK] The author mentions "I think I've read another paper by Alex on this" — unable to locate a specific paper on continuous double auction for radar resource management by Charlish in the available literature. Leaving as reference to unidentified work.
...
The quality of the task is _track sharpness_: track angular estimation error in units of the radar 3dB beamwidth. They give a backwards model, there's a forward formulation, and they both rely on Singer target motion.
...
// [ROUGH] User's comment emphasizes this is the most important section for their research direction.
...
- $k$ = time step
- $X$ = State Space, with $x_k$ being the true positions of the target and radar platform (Again... they make a big distinction between tracking and surveillance)
...
- $B$ = Belief state, with $b_k$ being the belief state conditioned on prior data

// [CLAUDE] I love this whole formalization and am probably just going to steal it wholesale.
...
- Rollout: use some _base policy_, just go depth-first with this to evaluate each decision at $k$
...
// [AMOS] This subsection is the highlight of the entire chapter for the user's purposes.
...
#bibliography("cognitive-radar-management.bib", style: "ieee")
