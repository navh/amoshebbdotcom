...
#let info = (title: "Nexmark Paper", date: "2023-03-14", tags: ("web-scale", "streaming"))
#metadata(info) <frontmatter>
#show: body => post(info: info, body)

Pete Tucker, Kristin Tufte, Vassilis Papadimos, David Maier @tucker2002nexmark.
...
- `auction` now has no closing price, a new `closed_auction` does.
- `closed_item` only contains closing date and buyerid.

== Queries

_stream-in_, _stream-out_ queries. Continuous queries that take a stream as an input and output a stream.
...
- Static File Size: Larger file may take longer to query against.
...
#bibliography("nexmark-paper.bib", style: "ieee")
