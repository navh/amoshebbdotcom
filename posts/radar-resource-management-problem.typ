...
#let info = (title: "The Radar Resource Management Problem", date: "2023-06-25", tags: ("radar",))
...
6-chapter book by Peter Moo and Zhen Ding @Moo2015.
...
- Aggregate target indication accuracies for all targets — geometric mean of the above for range, azimuth, elevation
...
2) distributed track fusion, each radar on its own, tell somebody central
3) distributed track maintenance, single track for each target, measurements from all radars
...
Basically channel is available with probability $p$. Down below they'll only ever set it to 1 and 0.5. Also, the way they did the intervals were in big blocks of 10 s at a time, flipping a coin every 10 s to see.
...
- Does confirmation look for the target
...
#bibliography("radar-resource-management-problem.bib", style: "ieee")
