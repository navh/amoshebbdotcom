...
#let info = (title: "Apache Flink 101", date: "2023-03-03", tags: ("web-scale",))
...
Or, you are processing _realtime data_, processing a stream of data and updating your model of the world.
...
Four types of time:
+ Event time — when user pressed button
+ Broker time — when event has been delivered to message queue (kafka)
+ Ingestion time — time Flink pulled message out of kafka
+ Window Processing time — when analyzed.
...
+ Master-worker model, one central highly available node appointed by zookeeper. Yarn or Mesos can deploy this for you. REST API or java or whatever to submit code or jar or whatever. Master takes care to distribute work across the workers.
+ Kubernetes model, containers which are usually workers, but there is a master image and one container will run the master.
+ Also Cloud DataProc.

#bibliography("apache-flink-101.bib", style: "ieee")
