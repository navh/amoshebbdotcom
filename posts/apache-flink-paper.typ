...
#let info = (title: "Apache Flink Paper", date: "2023-03-08", tags: ("web-scale",))
...
Flink was @Alexandrov2014[Stratosphere] from 2011 to 2014. I'll explore this further. Looks like it is mostly academic.
...
- Stateful Operators: split into parallel instances called _subtasks_
- Dataflows: split into _stream partitions_, point-to-point, broadcast, re-partition, fan-out, and merge.
...
Most others need new DAG for each iteration to be submitted. Flink has 'head' and 'tail' tasks that _implicitly_ connect feedback edges. (Cyclic DAG? They elaborate more in 4.4 and 5.3)
...
+ event-time - denotes the time when an event originates
+ processing-time - wall-clock time of machine processing data
+ ingestion-time - the time events enter Flink
+ real-time - wall-clock time for you and me
...
+ operator interfaces statically register explicit local variables
+ operator-state abstraction for declaring partitioned key-value states
+ Users can monkey with StateBackend

=== Stream Windows

Windows are stateful operators with three core functions: a window _assigner_, a _trigger_, and an _evictor_. There is a pool of common predefined implemetations, or a user can explicitly define their own function.

_Assigner_ assigns records to each window. For sliding windows, one element may belong to many.

_Trigger_ defines when operation will be performed.

_evictor_ determines which records to retain.
...
Interactive graph analytics. Bulk Synchronous Parallel, Stale Synchronous Parallel, and _delta_ iterations which can exploit sparse dependencies.
...
#bibliography("apache-flink-paper.bib", style: "ieee")
