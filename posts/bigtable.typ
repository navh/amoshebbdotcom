...
#let info = (title: "Bigtable", date: "2023-03-01", tags: ("web-scale", "jeff", "sanjay"))
...
Fay Chang, Jeffrey Dean, Sanjay Ghemawat, Wilson C. Hsieh, Deborah A. Wallach, Mike Burrows, Tushar Chandra, Andrew Fikes, Robert E. Gruber @chang2008bigtable
...
Arbitrary length on keys. (they pick 64 KB max, in practice, < 100 bytes typical). Every read or write under a row is atomic.

Column keys are grouped into _column families_. Unbounded number of cols in < 1000 families. Keys use syntax `family:qualifier`. Example here is `language`, storing multi-lingual websites in a row by URL and `language:english`, `language:french` columns.
...
Clients can write, delete, read, read-range, atomic read-modify-write, batching writes, auto-incrementers, running client-supplied scripts in address space of servers. Plays nice with Sawzall @pike2005sawzall, MapReduce @dean2004mapreduce.

== Building Blocks

SSTable is the file format. Persistent, ordered immutable map from keys to values. Internally, SSTable has 64 KB blocks. Block index is loaded into memory. Lookups are 1 disk seek. Binary search of in-memory index, go straight to block. SSTable can also be in memory.

Bigtable also relies on Chubby @burrows2006chubby (log that never dies). Leases, appointing master, bootstrapping location of data, discover tablet servers, finalize tablet deaths, schema info, access control lists. Chubby dies 0.0047% of the time, 0.0326% worst example.
...
As a table grows, it is automatically split into 100--200 MB tablets.
...
1. Master grabs _master lock_ in Chubby
2. Master scans servers dir in Chubby to find live servers
3. Master asks each server what they have
4. Master scans `METADATA` table to learn about all tablets, populates _unassigned_
5. Master knows about all `METADATA` tablets because it has read root tablet.
...
#bibliography("bigtable.bib", style: "ieee")
