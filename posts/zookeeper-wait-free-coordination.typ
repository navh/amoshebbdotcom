...
#let info = (title: "Zookeeper Wait Free Coordination", date: "2023-01-18", tags: ("web-scale",))
#metadata(info) <frontmatter>
#show: body => post(info: info, body)

The paper which describes ZooKeeper, a service for coordinating processes of distributed applications @hunt2010zookeeper.

== Bottom Line Up Front

I rate this 7.2.

ZooKeeper is hard to kill, clients think it is FIFO, writes are, reads are fast and loose. Sync allows cool tricks for when reads matter.
...
- Coordination recipes: Show how ZooKeeper can build primitives.
...
Linearizable writes. Well, asynchronous linearizability, one client may have multiple outstanding operations. This also satisfies linearizability. Write only allows service to scale linearly with servers added to the system.
...
Leader designates path as _ready_ znode, writes to it once config is ready. Other processes watch this. New leader makes changes. New leader deletes ready. Leader must change 5000 config znodes, 10 seconds sync, $<$1 second async. Others watch for ready, if they don't see it, leader must have died.

Notification order is guaranteed, so clients can't see a dead leader's ready while new one is making changes.
...
ZooKeeper is useful. Hundreds of thousands of ops/sec with fast reads. Wait-free property is performant.
...
#bibliography("zookeeper-wait-free-coordination.bib", style: "ieee")
